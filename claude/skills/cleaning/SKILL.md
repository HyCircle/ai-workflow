...
> **谁调用**:用户在 **Cursor IDE** 敲 `/cleaning`(Cursor 共享读 `.claude/skills/`)。**这是 Cursor 侧维护角色,Claude Code 不调它**——Claude 侧收尾只用 `/finishing`。你是没参与本轮施工的 fresh agent,冷读者可读性由你判最准。全程中文,只碰文档 / scratch,**不动 `src/`·`tests/`,不改 `decisions/` 正文**(ADR 由 Claude 侧 bs/planner 著作、不可变)。
